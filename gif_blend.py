...
def merge_images(bg, fg_alpha, s_x, s_y):
    ...
    result_img[s_y:f_h+s_y, s_x:f_w+s_x] = (result_img[s_y:f_h+s_y, s_x:f_w+s_x] * (1.0 - alpha)).astype('uint8') # アルファ以外の部分を黒で合成
    result_img[s_y:f_h+s_y, s_x:f_w+s_x] = (result_img[s_y:f_h+s_y, s_x:f_w+s_x] + (fg * alpha)).astype('uint8')  # 合成
...
def make_gif(im_list, fps):
    ...
    im_list[0].save(path, save_all=True, append_images=im_list[1:], duration=duration_time, loop=0)
